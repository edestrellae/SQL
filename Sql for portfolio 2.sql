...
select Customers.cust_contact, customers.cust_name, orderitems.order_num,
sum(orderitems.quantity * Orderitems.item_price) AS total_order_price
from customers join Orders
on customers.cust_id = Orders.cust_id
join orderitems on orders.order_num = Orderitems.order_num
group by Customers.cust_contact, orders.cust_id, customers.cust_name,
Orderitems.order_num
order by customers.cust_contact;
...
select Customers.cust_name, avg(orderitems.quantity * orderitems.item_price) As avg_order_value
from customers join orders
on customers.cust_id = Orders.cust_id
join Orderitems on orders.order_num = Orderitems.order_num
